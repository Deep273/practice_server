...
<div class="books-page">
    ...
    <form method="POST" action="/practic/delete-books">
        <input name="csrf_token" type="hidden" value="<?= app()->auth::generateCSRF() ?>"/>

        <div class="books-actions">
            <a href="<?= app()->route->getUrl('/create-book') ?>" class="btn add-book">Добавить книгу</a>
            <button type="submit" class="btn delete-book">Удалить выбранные книги</button>
...
<script>
    document.querySelector('.delete-book')?.addEventListener('click', function (e) {
        if (!confirm('Удалить выбранные книги?')) {
            e.preventDefault();
        }
    });
</script>